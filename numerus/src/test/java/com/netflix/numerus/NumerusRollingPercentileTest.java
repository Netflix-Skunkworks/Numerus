...
import com.netflix.numerus.NumerusRollingPercentile.PercentileSnapshot;
import com.netflix.numerus.NumerusRollingPercentile.Time;

public class NumerusRollingPercentileTest {

    private static final NumerusProperty<Integer> timeInMilliseconds = NumerusProperty.Factory.asProperty(60000);
    private static final NumerusProperty<Integer> numberOfBuckets = NumerusProperty.Factory.asProperty(12); // 12 buckets at 5000ms each
    private static final NumerusProperty<Integer> bucketDataLength = NumerusProperty.Factory.asProperty(1000);
    private static final NumerusProperty<Boolean> enabled = NumerusProperty.Factory.asProperty(true);

    @Test
    public void testRolling() {
        MockedTime time = new MockedTime();
        NumerusRollingPercentile p = new NumerusRollingPercentile(time, timeInMilliseconds, numberOfBuckets, bucketDataLength, enabled);
        p.addValue(1000);
        p.addValue(1000);
        p.addValue(1000);
        p.addValue(2000);
    ...
    public void testDoesNothingWhenDisabled() {
        MockedTime time = new MockedTime();
        int previousTime = 0;
        NumerusRollingPercentile p = new NumerusRollingPercentile(time, timeInMilliseconds, numberOfBuckets, bucketDataLength, NumerusProperty.Factory.asProperty(false));
        for (int i = 0; i < SampleDataHolder2.data.length; i++) {
            int timeInMillisecondsSinceStart = SampleDataHolder2.data[i][0];
            int latency = SampleDataHolder2.data[i][1];
            time.increment(timeInMillisecondsSinceStart - previousTime);
